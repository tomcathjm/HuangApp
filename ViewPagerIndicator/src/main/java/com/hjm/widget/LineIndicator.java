...
public class LineIndicator extends LinearLayout {
    ...
    private Paint mPaint; // 画笔---画绿色的指示条

    private Paint linePaint; // 底层的灰色固定条

    private int mLineWidth; // 绿色指示条的长度

    private int mInitTranslationX; // 初始化时指示条的平移距离

    private int mTranlationX;// 每一次滑动指示条的平移的距离
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        // 画 底部的灰色线条
        canvas.drawLine(0, getHeight() - 6, getScreenWidth() + mTranlationX+mInitTranslationX, getHeight() - 6, linePaint);

        // 画 绿色指示条
        canvas.drawLine(mInitTranslationX + mTranlationX, getHeight() -6, getScreenWidth() / mTabVisibleCount + mTranlationX - mInitTranslationX, getHeight() -6, mPaint);
